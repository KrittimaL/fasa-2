...
function AlertDetail() {
  ...
  async function updateCase(case_id, case_data) {
    const response = await fetch(`http://localhost:3001/update/${case_id}`, {
      method: "POST",
      headers: {
  ...
  function falsePositive() {
    ...
    updateCase(alertDetail._id, { ...alertDetail, response: false });
  ...
  function dispatch() {
    ...
    updateCase(alertDetail._id, { ...alertDetail, response: true });
    if (alertDetail.response) {
      navigate("/");
    }
  }
      ...
      <div className="w-full flex flex-col py-6 px-[120px] gap-5 mt-[60px]">
        <div className="flex w-full items-center justify-center">
          <p className="text-3xl font-semibold text-primary">
            Alert #{alertDetail.id}
          </p>
          {alertDetail.status ? (
            <p className="flex text-p font-bold ml-8 text-red">
              -{responseStatus}
            </p>
          ) : null}
        </div>
        <div className="flex flex-col w-full gap-4 ">
          <div className="flex w-full justify-center">
            <img
              ...
              src={focusImage ? focusImage : alertDetail.images[0]}
              alt={focusImage ? focusImage : alertDetail.images[0]}
            />
          </div>
          <div className="flex flex-row w-full items-center justify-center gap-3">
            {alertDetail.images.map((image, index) => {
              return (
                ...
                >
                  <img
                    className="max-w-[200px] max-h-[120px] aspect-square rounded-lg border border-[#D9D9D9] "
                    src={image}
                    alt={image}
                  />
                </button>
        ...
        <div className="border-t-2 border-t-[#D9D9D9] w-full flex flex-col pt-4 gap-6 ">
          ...
          <div className="flex w-full gap-3">
            ...
            <button
              className="py-1 px-1.5 rounded-lg bg-[#0C6937] hover:bg-[#095A2F] w-fit h-fit"
              onClick={dispatch}
            >
